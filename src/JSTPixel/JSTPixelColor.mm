...
- (NSString *)cssRGBAString {
    return
        [NSString stringWithFormat:@"rgba(%d,%d,%d,%.2f)", self.red, self.green, self.blue, (float)self.alpha / 0xff];
}
...
- (instancetype)initWithRed:(JST_COLOR_COMPONENT_TYPE)red
                      green:(JST_COLOR_COMPONENT_TYPE)green
                       blue:(JST_COLOR_COMPONENT_TYPE)blue
                      alpha:(JST_COLOR_COMPONENT_TYPE)alpha {
    self = [self init];
...
- (SystemColor *)toSystemColorWithColorSpace:(SystemColorSpace)colorSpace {
#if TARGET_OS_IPHONE
    NSAssert(CGColorSpaceGetModel(colorSpace) == kCGColorSpaceModelRGB ||
    ...
    if (CGColorSpaceGetModel(colorSpace) == kCGColorSpaceModelRGB) {
        ...
        CGFloat _gray = 0.299 * (CGFloat)_red / JST_COLOR_COMPONENT_MAX_VALUE +
                        0.587 * (CGFloat)_green / JST_COLOR_COMPONENT_MAX_VALUE +
                        0.114 * (CGFloat)_blue / JST_COLOR_COMPONENT_MAX_VALUE;
...
#if TARGET_OS_IPHONE
    NSAssert(CGColorSpaceGetModel(CGColorGetColorSpace(systemColor.CGColor)) == kCGColorSpaceModelRGB ||
                 CGColorSpaceGetModel(CGColorGetColorSpace(systemColor.CGColor)) == kCGColorSpaceModelMonochrome,
             @"unsupported color model");
#else
    NSAssert(systemColor.colorSpace.colorSpaceModel == NSColorSpaceModelRGB ||
                 systemColor.colorSpace.colorSpaceModel == NSColorSpaceModelGray,
             @"unsupported color model");
#endif
